...
y <- rexp(15, rate = 3)
eca.test(y, dist = "exp", params = list(rate = 2))
...
dist="norm"
params_list <- list(mean = 100, sd = 5)
## Tu dane i formalny test
n=10
x <- rnorm(n,100,5)
eca.test(x, dist = dist, params = params_list, delta = 1/(2*n))

eca.test(x, dist = "norm", params = list(mean = 100, sd = 5), delta = 1/(2*n), plot = TRUE)



## Wyznaczanie wartości krytycznych testu
xm_dane <-matrix(rnorm(n*1000, mean = 100, sd = 5),1000,n )
pc=apply(xm_dane,1,ecm)
quantile(pc,c(0.9,0.95,0.99))



dist="unif"
params_list <- list(min=0,max=10)
n=10

x <- runif(n,min=0,max=10) 
wynik <- eca.test(x, dist = dist, params = params_list, delta = 1/(2*n), plot = TRUE)
print(wynik)

xm_dane <-matrix(runif(n*1000, 0,10),1000,n )
pc=apply(xm_dane,1,eca.test)
quantile(pc,c(0.9,0.95,0.99))



dist="exp"
params_list <- list(rate=2)
n=10

x <- rexp(n,rate=3) 
wynik <- eca.test(x, dist = "exp", params = params_list, delta = 1/(2*n), plot = TRUE)
print(wynik)
